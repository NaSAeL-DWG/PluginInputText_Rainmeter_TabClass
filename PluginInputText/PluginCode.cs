...
namespace InputText
{
    internal partial class Measure
    {
        private bool ReadOptions(ExecuteBangParam param)
        {
            ...
            ReadOption("Scale", param.Options);
        ...
        private string GetUserInput(Dictionary<string, string> Options, Dictionary<string, string> Overrides)
        {
            ...
            lock (this._InputBoxLocker)
            {
                ...
                double scale = 1.0; 
                    ...
                    {
                        double.TryParse(Overrides["Scale"], out scale); // 使用 double.TryParse
                    }
                    else if (Options.ContainsKey("Scale"))
                    {
                        double.TryParse(Options["Scale"], out scale); // 使用 double.TryParse
                    }
                ...
                changeSetting("X", (value) => input.ChangeX(value, scale));
                changeSetting("Y", (value) => input.ChangeY(value, scale));
